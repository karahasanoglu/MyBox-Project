#!/bin/bash
# ============================================================
# MyBox - Gelişmiş Kurulum ve Yetkilendirme Scripti (v1.0.7)
...
echo "          |___/  Container Runtime v1.0.7"
echo -e "${NC}"
...
# Eger proje dizininde degilsek (curl | bash calistiriliyorsa), gecici dizine klonla
if [ ! -f "go.mod" ] || [ ! -d "cmd/main" ]; then
    echo -e "${YELLOW}[*] Proje kaynak kodu bulunamadı. Web üzerinden kurulum başlatılıyor...${NC}"
    WORK_DIR=$(mktemp -d)
    # Temizlik islemini scriptin sonunda veya hata durumunda calisacak sekilde ayarla
    trap 'rm -rf "$WORK_DIR"' EXIT
    cd "$WORK_DIR"
    
    # Git yuklu degilse yukle
